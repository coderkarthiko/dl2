...
class optim:
    ...
    def zero(self):
        self.dW = np.array([np.zeros(np.shape(w)) for w in self.W])
        self.dB = np.array([np.zeros(np.shape(b)) for b in self.B])
...
class Adam(optim):
    ...
    def step(self):
        ...
        self.t += 1
        if not self.accumlate:
            self.zero()
        return self.W, self.B
    
    def reset(self):
        self.zero()
        self.vW, self.vB, self.mW, self.mB = self.dW, self.dB, self.dW, self.dB
...
class Model:
    def __init__(self, sizes, actvns):
        ...
        self.n = len(sizes)
        # some more stuff
        self.compile, self.reset = False, False
        self.optim, self.loss_fn = None, None
        
    # initialize network for training
    def comp(self, optim, loss_fn, *args):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for x_batch, y_batch in zip(x, y):
                ...
                for xi, yi in zip(x_batch, y_batch):
                    out = self.forward(xi)
                    error, loss = self.dl(yi, loss_fn), self.loss(yi, loss_fn)
