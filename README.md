...
*4. The website didn't really go over how to actually compute the softmax gradient matrix - all we need to do is compute the outer product of softmax layer with itself, multiply by negative one, multiply trace of the square matrix by 2, take the sum of elements in each row and we end up with the derivative of the objective function w.r.t inputs of the softmax layer (a.k.a error in the softmax layer). Then you backpropagate the error. I found it much easier to understand ML algorithms like backpropagation and GD when I implemented them rather than simply looking at the mathematics behind them...*
